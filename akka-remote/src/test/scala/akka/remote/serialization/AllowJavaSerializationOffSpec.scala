...
class AllowJavaSerializationOffSpec
  ...
  "Disabling java serialization" should {
    ...
    "have replaced java serializer" in {
      ...
      val serializer2 = SerializationExtension(noJavaSerializationSystem).findSerializerFor(new ProgrammaticJavaDummy)
      serializer2.getClass should ===(classOf[DisabledJavaSerializer])
      serializer2.identifier should ===(serializer.identifier)

      SerializationExtension(system).verifySerialization(new ProgrammaticDummy)
      SerializationExtension(dontAllowJavaSystem).verifySerialization(new ProgrammaticDummy)
    }
